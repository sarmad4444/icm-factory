...
def get_telemetry_snippet(style: str) -> str:
    """Returns the mandatory response telemetry protocol snippet for injection into Section 5."""
    if style == "pill-bar":
        return (
...
def inject_skills(target: Path):
    ...
    # Auto-seed .graphifyignore and install git hooks if graphify is installed
    if (target_skills / "graphify").is_dir():
        ignore_file = target / ".graphifyignore"
        if not ignore_file.exists():
            ignore_file.write_text(
                "# Graphify ignore patterns for sub-second AST extraction\n"
                ".venv/\n"
                ".worktrees/\n"
                "node_modules/\n"
                "output/\n"
        ...
        if (target / ".git").is_dir():
            try:
                import subprocess
                subprocess.run(["graphify", "hook", "install"], cwd=str(target), capture_output=True, check=False)
            except Exception:
                pass
...
def inject_pm(target: Path, workspace_name: str):
    ...
    if strat_tmpl.is_file():
        ...
        strat_content = f"""# Project Strategy & Live Status
...
def scaffold_workspace(
    ...
    if topo_num in [1, 2]:
        ...
        for idx, s in enumerate(stage_list, start=1):
            ...
            contract = f"""# Stage {idx:02d}: {s_title}
    ...
    # Scaffold multi-workflow stages (Topologies 3 & 4)
        ...
        for wf_name, wf_stages in wf_dict.items():
            ...
            for idx, s in enumerate(wf_stages, start=1):
                ...
                contract = f"""# {wf_name.title()} — Stage {idx:02d}: {s_title}
...
"""
    ...
    telemetry_header = get_telemetry_snippet(telemetry_style)

    agent_tmpl = get_template_path("AGENTS")
    if not agent_tmpl.is_file():
    ...
    if agent_tmpl.is_file():
        agent_content = (
            ...
            .replace("{TELEMETRY_HEADER_SNIPPET}", telemetry_header)
        )
    else:
    ...
    claude_content = (
        f"# {name} — Claude Code Operating Guide\n\n"
        f"**Framework:** Interpretable Context Methodology (ICM)\n\n"
        f"Please refer to [`AGENTS.md`](file://./AGENTS.md) for operating rules and floor plan.\n"
        f"Refer to [`CONTEXT.md`](file://./CONTEXT.md) for task routing.\n"
    )
    (target / "CLAUDE.md").write_text(claude_content, encoding="utf-8")

    gemini_content = (
        f"# {name} — Google Antigravity Operating Guide\n\n"
        f"**Framework:** Interpretable Context Methodology (ICM)\n\n"
        f"Please refer to [`AGENTS.md`](file://./AGENTS.md) for operating rules and floor plan.\n"
        f"Refer to [`CONTEXT.md`](file://./CONTEXT.md) for task routing.\n"
    )
    (target / "GEMINI.md").write_text(gemini_content, encoding="utf-8")
...
def scaffold_from_archetype(
    ...
    source_file = agents_file if agents_file.is_file() else agent_file
...
def main():
    ...
    parser.add_argument(
        "--telemetry-style",
        choices=["none", "pill-bar", "terminal-box", "cockpit-hud", "monospace-grid", "micro-table"],
        default="pill-bar",
        help="Execution telemetry style for AGENTS.md (default: pill-bar)",
    )
