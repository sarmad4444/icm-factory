...
**Domain:** Interpretable Context Methodology (ICM) Workspace Generation, Tooling & Governance
...
### Category 2: Project Management & Objective Sprints
| Goal / Intent | Action / Tool | Command / Contract | Expected Output |
|---|---|---|---|
...
| Compile 5-Part Prompt Initiative | Prompt Compiler | Read [`resources/templates/shaped_initiatives.md.tmpl`](file://./resources/templates/shaped_initiatives.md.tmpl) | Executable prompt contract |
...
- `software_feature`: 4-stage Specification $\rightarrow$ TDD $\rightarrow$ Implementation $\rightarrow$ Verification
- `system_architecture_rfc`: 5-stage Problem Discovery $\rightarrow$ Tradeoff Matrix $\rightarrow$ Diagrams $\rightarrow$ Migration Plan $\rightarrow$ RFC
- `systematic_bug_triage`: 4-stage Reproduction $\rightarrow$ Root-Cause $\rightarrow$ Regression Test $\rightarrow$ Fix & Review
- `agile_software_engine`: Full Topology 2/4 engine with `docs/STRATEGY.md`, sprint phases, and prompt compiler
